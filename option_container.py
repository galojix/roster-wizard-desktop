...
class ExampleOptionContainerApp(toga.App):
    ...
    def on_change_option_title(self, button):
        ...
        self.optioncontainer.content[
            index
        ].label = self.input_change_title.value
    ...
    def startup(self):
        ...
        # select
        label_select = toga.Label(
            "Select an Option position:", style=style_flex
        )
        ...
        box_select = toga.Box(
            style=style_select, children=[label_select, self.select_option]
        )
        box_actions_col1 = toga.Box(
            style=style_row, children=[btn_remove, btn_enabled]
        )
        ...
        box_actions = toga.Box(
            style=style_col, children=[box_actions_col1, box_actions_col2]
        )
        box_container_actions = toga.Box(
            style=style_row, children=[box_select, box_actions]
        )
        ...
        box_general_actions = toga.Box(
            style=Pack(padding_bottom=10), children=[btn_add]
        )
